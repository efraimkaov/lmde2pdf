...
from modules.lmdeTagsBilingual import lmdeTagsBilingual_func

def contentXmlBilingual_func(directory, installationDir, template, pageNumber, defaultStyle, lmdeName, leftHeaderValidate, rightHeaderValidate):
    ...
    contentXmlTemplate = open(installationDir+'/templates/'+template+'/content-table.xml', 'r')
    for line in contentXmlTemplate.readlines():
        contentXml.write(line.replace('#pageNumber#', str(pageNumber)))
    contentXmlTemplate.close()
    ...
    contentXml.write('<style:paragraph-properties/>\n')
    ...
    contentXml.write('<table:table table:name="Table1" table:style-name="Table1">\n')
    contentXml.write('<table:table-column table:style-name="Table1.A"/>\n')
    contentXml.write('<table:table-column table:style-name="Table1.B"/>\n')
    contentXml.write('<table:table-column table:style-name="Table1.A"/>\n')
    ...
    while defaultStyleCount < contentDefaultStyleLen:
        ...
        if defaultStyleCount == defaultStyleCountCheck:
            tagVar = 'P1'
            lmdeTagsBilingual_func(directory, contentDefaultStyleAll, contentDefaultStyleVar, contentXml, tagVar, defaultStyleCount)
